...
// https://green-api.com/telegram/docs/api/receiving/notifications-format/incoming-message/Webhook-IncomingMessageReceived/
// Параметры,
...
// https://green-api.com/telegram/docs/api/receiving/notifications-format/outgoing-message/OutgoingMessageStatus/
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
//  Данные - Структура - Структура полей сообщения в формате JSON. См подробнее в документации
...
// https://green-api.com/telegram/docs/api/receiving/notifications-format/StateInstanceChanged/
//
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
...
// https//green-api.com/telegram/docs/api/receiving/notifications-format/DeviceInfo/
//
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
...
//	Подробнее https://green-api.com/telegram/docs/api/account/StartAuthorization/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Используйте код проверки полученный при вызове метода StartAuthorization.
//
// Параметры:
//	 КодИзЧата - строка - код который получил 
//   Пароль - строка - пароль от аккаунта (при наличии двухфакторной аутентификациии)
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьКодАвторизации(КодИзЧата, Пароль = "") Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Структура = Новый Структура;
	Структура.Вставить("code", КодИзЧата);
...
//
// Параметры:
//   Пароль - строка - пароль от аккаунта
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьПарольАвторизации(Пароль) Экспорт
	...
	Ответ = ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "sendAuthorizationPassword"), Тело);
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для разлогинивания инстанса.
//	Подробнее https://green-api.com/telegram/docs/api/account/Logout/ 
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Подробнее https://green-api.com/telegram/docs/api/account/GetStateInstance/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Подробнее https://green-api.com/telegram/docs/api/account/Reboot/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Подробнее https://green-api.com/telegram/docs/api/account/GetSettings/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Метод предназначен для получения текущих настроек инстанса.
//	Подробнее https://green-api.com/telegram/docs/api/account/setSettings/
//
// Параметры:
...
//	Подробнее https://green-api.com/telegram/docs/api/account/SetProfilePicture/
//
// Параметры:
...
Функция УстановитьФотоПрофиля(ПутьКФайлу) Экспорт
	...
	БинРазделитель = ПолучитьДвоичныеДанныеИзСтроки("--" + Разделитель + CRLF, КодировкаТекста.UTF8, Ложь);
	БинРазделительФиниш = ПолучитьДвоичныеДанныеИзСтроки(CRLF + "--" + Разделитель + "--", КодировкаТекста.UTF8, Ложь);
	
	Тело = Новый ПотокВПамяти();
	ЗаписьДанных = Новый ЗаписьДанных(Тело);
	
	ЗаписьДанных.Записать(БинРазделитель);
	ЗаписьДанных.Записать(ФайлFormData("file", Файл, ДвоичныеДанные));
	
	ЗаписьДанных.Записать(БинРазделительФиниш);
...
// Метод предназначен для получения информации об аккаунте мессенджера Telegram.
//	Подробнее https://green-api.com/telegram/docs/api/account/GetAccountSettings/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/sending/SendMessage/
//
// Параметры:
...
// Метод предназначен для отправки файла напрямую (Multipart)
Функция ОтправитьВидеоАудиоИзображениеДокумент(ИдЧата, ПутьКФайлу, Заголовок = "") Экспорт
	
	Инстанс = IdInstance;
	Токен   = ApiToken;
	// ВАЖНО: Тут должен вернуться "4100.api.green-api.com" согласно вашему скриншоту
	...
	// Генерируем границу (Boundary)
	Boundary = СтрЗаменить(Строка(Новый УникальныйИдентификатор()), "-", "");
	СтрокаBoundary = "--" + Boundary;
	
	// Служебные символы
	CRLF = Символы.ВК + Символы.ПС;
	
	// Создаем поток для сборки тела
	ПотокТела = Новый ПотокВПамяти();
	ЗаписьДанных = Новый ЗаписьДанных(ПотокТела);
	
	// 1. Добавляем chatId
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(СтрокаBoundary + CRLF, КодировкаТекста.UTF8, Ложь));
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки("Content-Disposition: form-data; name=""chatId""" + CRLF + CRLF, КодировкаТекста.UTF8, Ложь));
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(ИдЧата + CRLF, КодировкаТекста.UTF8, Ложь));
	
	// 2. Добавляем заголовок (caption), если есть
	Если ЗначениеЗаполнено(Заголовок) Тогда
		ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(СтрокаBoundary + CRLF, КодировкаТекста.UTF8, Ложь));
		ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки("Content-Disposition: form-data; name=""caption""" + CRLF + CRLF, КодировкаТекста.UTF8, Ложь));
		ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(Заголовок + CRLF, КодировкаТекста.UTF8, Ложь));
	КонецЕсли;

	// 3. Добавляем сам ФАЙЛ
	// Важно: имя файла лучше передавать транслитом или латиницей, если сервер капризный, 
	// но мы попробуем передать как есть в UTF-8.
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(СтрокаBoundary + CRLF, КодировкаТекста.UTF8, Ложь));
	
	ЗаголовокФайла = "Content-Disposition: form-data; name=""file""; filename=""" + ФайлНаДиске.Имя + """" + CRLF +
					 "Content-Type: " + ПолучитьMimeTypeПоРасширению(ФайлНаДиске.Расширение) + CRLF + CRLF;
					 
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(ЗаголовокФайла, КодировкаТекста.UTF8, Ложь));
	ЗаписьДанных.Записать(ДвоичныеДанныеФайла);
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(CRLF, КодировкаТекста.UTF8, Ложь));
	
	// 4. Финализируем тело
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(СтрокаBoundary + "--" + CRLF, КодировкаТекста.UTF8, Ложь));
	
	ЗаписьДанных.Закрыть();
	ДанныеТела = ПотокТела.ЗакрытьИПолучитьДвоичныеДанные();
	
	// Отправляем
	Заголовки = Новый Соответствие();
	Заголовки.Вставить("Content-Type", "multipart/form-data; boundary=" + Boundary);
	
	// Используем нашу исправленную функцию с логами из прошлого шага
...
// Подробнее https://green-api.com/telegram/docs/api/sending/SendFileByUrl/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
Функция ОтправитьВидеоАудиоИзображениеДокументПоURL(ИдЧата, УРЛФайла, ИмяФайла, Заголовок = "") Экспорт
	...
	Возврат ОтправитьPOSTЗапрос(Хост, URLМетодаСервиса(Инстанс, Токен, "sendFileByUrl"), Тело);
...
// Подробнее https://green-api.com/telegram/docs/api/sending/UploadFile/
//
// Параметры:
...
//  ЧтениеJSON  - Ответ сервера
//
// Метод загружает файл в облако и возвращает ссылку
...
// Подробнее https://green-api.com/telegram/docs/api/sending/SendLocation/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
// Подробнее https://green-api.com/telegram/docs/api/sending/SendContact/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьКонтакт(ИдЧата, Телефон, Имя, Фамилия = "") Экспорт
	...
	// В документации GreenAPI для Telegram поле называется phoneContact (число) или phoneNumber
	// Используем то, что вы написали, но убедитесь, что API принимает именно "phoneNumber"
	Контакт.Вставить("phoneContact", Телефон); 
	...
	// Обратите внимание: метод API называется sendContact
	Возврат ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "sendContact"), Тело);
	
КонецФункции

Функция ОтправитьОпрос(ChatId, Вопрос, ВариантыМассив, МножественныйВыбор = Ложь) Экспорт
	Структура = Новый Структура;
	Структура.Вставить("chatId", ChatId);
	Структура.Вставить("message", Вопрос);
	
	МассивОпций = Новый Массив;
	Для Каждого ТекВариант Из ВариантыМассив Цикл
		МассивОпций.Добавить(Новый Структура("optionName", ТекВариант));
	КонецЦикла;
	
	Структура.Вставить("options", МассивОпций);
	Структура.Вставить("multipleAnswers", МножественныйВыбор);
	
	Тело = СериализоватьВJSONСтроку(Структура);
	Возврат ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(IdInstance, ApiToken, "sendPoll"), Тело);
КонецФункции
...
// Подробнее https://green-api.com/telegram/docs/api/receiving/technology-http-api/ReceiveNotification/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/receiving/files/DownloadFile/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
// Подробнее https://green-api.com/telegram/docs/api/journals/GetChatHistory/
// 
// Параметры:
//  ИдЧата - Строка - идентификатор чата	
...
// Подробнее https://green-api.com/telegram/docs/api/journals/GetMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор чата
//  ИдСообщения - Строка - Идентификатор сообщения
...
// Подробнее https://green-api.com/telegram/docs/api/journals/LastIncomingMessages/
//
// Параметры:
...
// Подробнее https://green-api.com/telegram/docs/api/journals/LastOutgoingMessages/
// 
// Параметры:
//	Интервал - число - Время в минутах, за которое требуется показать сообщения (по умолчанию равно 1440 минут)
//
...
// Метод предназначен для получения количества сообщений в очереди на отправку в мессенджере Telegram.
//	Каждое уведомление хранится в очереди 24 часа. Информация в методе GetMessagesCount обновляется 1 раз в 10 секунд.
// Подробнее https://green-api.com/telegram/docs/api/queues/GetMessagesCount/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/queues/ShowMessagesQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/queues/ClearMessagesQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Каждое уведомление хранится в очереди 24 часа. Информация в методе GetMessagesCount обновляется 1 раз в 10 секунд.
// Подробнее https://green-api.com/telegram/docs/api/queues/GetWebhooksCount/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/queues/ClearWebhooksQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/groups/CreateGroup/
//
// Параметры:
//  ИмяГруппы - Строка - Наименование нового группового чата
//  ИдЧатов - Массив - Коллекция ид чатов(строки) участников группы
//  ТипГруппы - Строка - Тип группы (group / supergroup)
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция СоздатьГруппу(ИмяГруппы, ИдЧатов, ТипГруппы = "supergroup") Экспорт
	...
	Структура.Вставить("type", ТипГруппы);
...
// Подробнее https://green-api.com/telegram/docs/api/groups/UpdateGroupName/
//
// Параметры:
...
// Подробнее https://green-api.com/telegram/docs/api/groups/GetGroupData/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//
...
// Подробнее https://green-api.com/telegram/docs/api/groups/AddGroupParticipant/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
...
// Подробнее https://green-api.com/telegram/docs/api/groups/RemoveGroupParticipant/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//  ИдУчастника - Строка - Идентификатор участника, добавляемого в групповой чат.
...
// Подробнее https://green-api.com/telegram/docs/api/groups/SetGroupAdmin/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
...
// Подробнее https://green-api.com/telegram/docs/api/groups/RemoveAdmin/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
...
// Подробнее https://green-api.com/telegram/docs/api/groups/LeaveGroup/
//
// Параметры:
...
// Подробнее https://green-api.com/telegram/docs/api/groups/SetGroupPicture/
//
// Параметры:
...
Функция УстановитьФотоГруппы(ИдГруппы, ПутьКФайлу) Экспорт
	...
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(СтрокаBoundary + CRLF, КодировкаТекста.UTF8, Ложь));
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки("Content-Disposition: form-data; name=""chatId""" + CRLF + CRLF, КодировкаТекста.UTF8, Ложь));
	ЗаписьДанных.Записать(ПолучитьДвоичныеДанныеИзСтроки(ИдГруппы + CRLF, КодировкаТекста.UTF8, Ложь));
...
// Подробнее https://green-api.com/telegram/docs/api/marks/ReadChat/
//
// Параметры:
...
// Метод проверяет наличие аккаунта Telegram на номере телефона.
// Подробнее https://green-api.com/telegram/docs/api/service/CheckAccount/
//
// Параметры:
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ПроверитьНаличиеТелеграм(Телефон) Экспорт
...
// Подробнее https://green-api.com/telegram/docs/api/service/GetAvatar/
//
// Параметры:
...
// Подробнее https://green-api.com/telegram/docs/api/service/GetContacts/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/service/GetContactInfo/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/api/service/DeleteMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
...
// Подробнее https://green-api.com/telegram/docs/api/service/EditMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
...
// Подробнее https://green-api.com/telegram/docs/api/service/GetChats/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
// 
...
Функция РедактироватьСообщение(ChatId, IdMessage, Текст) Экспорт
	Структура = Новый Структура("chatId, idMessage, message", ChatId, IdMessage, Текст);
	Тело = СериализоватьВJSONСтроку(Структура);
	Возврат ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(IdInstance, ApiToken, "editMessage"), Тело);
КонецФункции
...
// Подробнее https://green-api.com/telegram/docs/partners/getInstances/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/telegram/docs/partners/deleteInstanceAccount/
//
// Параметры:  
...
Функция ОтправитьФайлPOSTЗапрос(Хост, Адрес, Заголовки, ТелоЗапроса)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("GreenAPI.ОшибкаСети", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		ВызватьИсключение "Ошибка соединения: " + ТекстОшибки;
	КонецПопытки;
	...
	Иначе
		...
		Если ПустаяСтрока(ТекстОтвета) Тогда
			ТекстОшибки = СтрШаблон("Ошибка HTTP %1. Тело ответа пустое.", Ответ.КодСостояния);
		Иначе
			ТекстОшибки = СтрШаблон("Ошибка HTTP %1. Ответ: %2", Ответ.КодСостояния, ТекстОтвета);
		КонецЕсли;
		
		// Пишем в Журнал Регистрации (Администрирование -> Журнал регистрации)
		ЗаписьЖурналаРегистрации("GreenAPI.ОшибкаЗапроса", 
			УровеньЖурналаРегистрации.Ошибка, , , 
			"Адрес: " + Адрес + Символы.ПС + ТекстОшибки);
...
Функция ПараметрFormData(ИмяПоля, Значение)
	
	CRLF = Символы.ВК + Символы.ПС;
	СтрокаДанных = "Content-Disposition: form-data; name=""" + ИмяПоля + """" + CRLF + CRLF + Значение;
	
	Возврат ПолучитьДвоичныеДанныеИзСтроки(СтрокаДанных, КодировкаТекста.UTF8, Ложь);
	
КонецФункции

Функция ФайлFormData(ИмяПоля, Файл, ДвоичныеДанныеФайла)
	
	CRLF = Символы.ВК + Символы.ПС;
	
	СтрокаЗаголовка = "Content-Disposition: form-data; name=""" + ИмяПоля + """; filename=""" + Файл.Имя + """" + CRLF +
					  "Content-Type: " + ПолучитьMimeTypeПоРасширению(Файл.Расширение) + CRLF + CRLF;
	
	БинЗаголовки = ПолучитьДвоичныеДанныеИзСтроки(СтрокаЗаголовка, КодировкаТекста.UTF8, Ложь);
	
	Поток = Новый ПотокВПамяти();
	Запись = Новый ЗаписьДанных(Поток);
	
	Запись.Записать(БинЗаголовки);
	Запись.Записать(ДвоичныеДанныеФайла);
...
Функция ХостПоУмолчанию() Экспорт
	...
	Если ЗначениеЗаполнено(host) Тогда	
		Результат = host;
	Иначе 
		Результат = "4100.api.green-api.com";
	КонецЕсли;      
...
Функция ВерсияОбработки() Экспорт
	Возврат "Green API-Telegram v" + НомерВерсии();
КонецФункции
